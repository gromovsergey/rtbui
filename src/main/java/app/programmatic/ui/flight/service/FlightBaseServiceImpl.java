...
public abstract class FlightBaseServiceImpl {
    ...
    public Long findAccountIdByFlightId(Long flightId) {
        return jdbcOperations.queryForObject("select account_id from insertionorder " +
                                             "  inner join flight using(io_id) where flight_id = ?",
                new Object[] { flightId },
                Long.class);
    }
    ...
    protected void setIdsForExistingSchedules(FlightBase flightBase, FlightBase existingBase) {
        ...
        flightScheduleRepository.delete(existingBase.getSchedules().stream()
