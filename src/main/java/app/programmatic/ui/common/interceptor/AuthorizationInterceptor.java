...
public class AuthorizationInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        if (    request.getRequestURI() == null ||
                !(request.getRequestURI().startsWith("/api/kickcaptcha/") || request.getRequestURI().startsWith("/rest/")) ||
                request.getRequestURI().startsWith("/rest/dynamicConfig/properties") ||
                request.getRequestURI().equals("/rest/login") ||
                request.getRequestURI().equals("/api/kickcaptcha/v1/user/login") ||
                request.getRequestURI().equals("/api/kickcaptcha/v1/user/registration") ||
                request.getRequestURI().equals("/api/kickcaptcha/v1/user/resetPassword") ||
        ...
        if (request.getRequestURI().startsWith("/api/kickcaptcha/")) {
            return authKickCaptcha(request, response);
        }
    ...
    private boolean authKickCaptcha(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String[] keyTokenArray = null;
        String keyTokenPair = request.getHeader(AUTH_HEADER);
        ...
        if (authProvided) {
            authProvided = kickCaptchaAuthorizationServiceImpl.configure(
                    keyTokenArray[0], request.getRemoteAddr(), USER_SESSION_TIMEOUT_IN_MINUTES)
                    != null;
        }
    ...
    public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler, ModelAndView modelAndView) throws Exception {
        ...
        kickCaptchaAuthorizationServiceImpl.cleanUp();
    }
